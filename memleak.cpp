...
int test1(int *data, int len){
  int sum = 0;
  for(int i=0; i < len; i++){ // change from <= to < because the former acceses information outside the array
    sum += data[i];
  }
  return sum;
}
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; // '+1' is deleted because it accesses memory outside of the array
  return otherword; // Do not have to delete the word because it will be deleted in the main
}
